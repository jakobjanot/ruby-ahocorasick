...
describe KeywordTree do
    ...
    describe "not bugs" do
        ...
        it "should return 2 results (the longest in each position)" do
            tree = KeywordTree.new
            tree.add_string "data"
            tree.add_string "database"
            results= tree.find_longest "move all the data to a new database"
            results.size.should == 2
            results[0][:value].should == "data"
            results[1][:value].should == "database"
        end
        it "should return 2 results (the smallest in each position)" do
            tree = KeywordTree.new
            tree.add_string "data"
            tree.add_string "database"
            results= tree.find_shortest "move all the data to a new database"
            results.size.should == 2
            results[0][:value].should == "data"
            results[1][:value].should == "data"
        end
    end
